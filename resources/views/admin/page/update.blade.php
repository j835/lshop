...
@section('content')
    ...
    <form action="{{ route('admin.page.update', ['id' => $page->id]) }}" method="POST" id="create-page-form" class="col-12">
        @csrf
        @method('POST')
        ...
        <input class="form-control no-margin @error('code') is-invalid @enderror" id="url-input" type="text" name="code"
            value="{{ old('code') ? old('code') : $page->code }}">
        <small class="form-text text-muted">Образец - /test , "/" добавляется автоматически при необходимости</small>
        <br>
        
        <label class="form-label mb-2 mt-3">Содержимое страницы</label>
        <textarea name="content" class="form-control @error('content') is-invalid @enderror" id="contentTextarea" rows="12">{{ old('content') ? old('content') : $page->content }}
        </textarea>

        <label class="form-label " style="margin-top:20px;">SEO description</label>
        <input class="form-control @error('seo_description') is-invalid @enderror" type="text" name="seo_description"
            value="{{ old('seo_description') ? old('seo_description') : $page->seo_description}}">

        <label class="form-label ">SEO keywords</label>
        <input class="form-control @error('seo_keywords') is-invalid @enderror" type="text" name="seo_keywords"
            value="{{ old('seo_keywords') ? old('seo_keywords') : $page->seo_keywords }}">
    ...
    <script>
        ...
        formSubmitConfirm('pageDeleteForm', 'Вы действительно хотите навсегда удалить страницу? Восстановление невозможно');
